...
impl<'a> Control<'a> {
    ...
    pub async fn join(&mut self, ssid: &str, options: JoinOptions<'_>) -> Result<(), JoinError> {
        self.join_inner(ssid, options, None).await
    }

    /// Join a network while toggling a CYW43 GPIO as association events are pending.
    ///
    /// This is useful for the Pico W onboard LED, which shares this control
    /// handle and therefore cannot otherwise be updated while `join` awaits.
    pub async fn join_with_gpio_flash(
        &mut self,
        ssid: &str,
        ...
        assert!(gpio_n < 3);
        self.join_inner(ssid, options, Some((gpio_n, interval, initial_on)))
            .await
    }
    ...
    async fn join_inner(
        ...
        ssid: &str,
        options: JoinOptions<'_>,
        gpio_flash: Option<(u8, Duration, bool)>,
    ) -> Result<(), JoinError> {
        self.set_iovar_u32("ampdu_ba_wsize", 8).await;
        ...
        self.wait_for_join(i, secure_network, gpio_flash).await
    }
    ...
    async fn wait_for_join(
        ...
        i: SsidInfo,
        secure_network: bool,
        mut gpio_flash: Option<(u8, Duration, bool)>,
    ) -> Result<(), JoinError> {
        ...
        let _uod = UnsubscribeOnDrop(events);
        events.mask.enable(&[Event::SET_SSID, Event::AUTH, Event::PSK_SUP]);
        let mut subscriber = events.queue.subscriber().unwrap();
        ...
        let result = loop {
            let msg = loop {
                let Some((gpio_n, interval, gpio_on)) = gpio_flash.as_mut() else {
                    break subscriber.next_message_pure().await;
                };

                match select(subscriber.next_message_pure(), Timer::after(*interval)).await {
                    Either::First(msg) => break msg,
                    Either::Second(()) => {
                        *gpio_on = !*gpio_on;
                        self.gpio_set(*gpio_n, *gpio_on).await;
                    }
                }
            };
